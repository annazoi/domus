...
export default function EarningsPage() {
	...
	return (
		<div className="space-y-10">
			...
			<section className="relative overflow-hidden rounded-2xl bg-white/80 p-6 md:p-8">
				<div
				...
				{loading ? (
					<div className="rounded-2xl bg-white/80">
						{Array.from({ length: 3 }).map((_, index) => (
							<div
								key={index}
								className="flex items-center justify-between border-b border-black/5 px-5 py-4 last:border-b-0"
							>
				...
				{!loading && stats.recentTransactions.length > 0 ? (
					<div className="rounded-2xl bg-white/80">
						{stats.recentTransactions.map((tx) => (
							<div key={tx.bookingId} className="flex items-center justify-between border-b border-black/5 px-5 py-4 last:border-b-0">
								<div>
